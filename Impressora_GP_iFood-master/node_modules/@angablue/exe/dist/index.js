...
async function exe(options) {
    ...
    if (options.skipBundle) {
        ...
        code = await fs_1.promises.readFile((0, path_1.resolve)(options.entry), 'utf8');
    }
    else {
        // Bundle with ncc
        const output = await (0, ncc_1.default)((0, path_1.resolve)(options.entry), {
            minify: true,
            quiet: true,
            target: 'es2021'
        });
    ...
    // Generate .exe
    await (0, utils_1.execAsync)(`node -e "require('fs').copyFileSync(process.execPath, '${out}')"`);
    ...
    const data = await fs_1.promises.readFile(options.out);
    ...
    // Add icon
    if (options.icon) {
        const iconFile = RE.Data.IconFile.from(await fs_1.promises.readFile(options.icon));
    ...
    await fs_1.promises.writeFile(options.out, Buffer.from(executable.generate()));
}
module.exports = exe;
